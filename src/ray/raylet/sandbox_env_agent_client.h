...
/// \param[in] serialized_sandbox_env_context Serialized context.
/// \param[in] setup_error_message The error message if runtime env creation fails.
/// It must be only set when successful == false.
using GetOrCreateSandboxEnvCallback =
    std::function<void(bool successful,
                       const std::string &serialized_sandbox_env_context,
                       const std::string &setup_error_message)>;
using DeleteSandboxEnvIfPossibleCallback = std::function<void(bool successful)>;

// Interface to talk to SandboxEnvManager.
class SandboxEnvAgentClient {
 public:
  ...
  static std::unique_ptr<SandboxEnvAgentClient> Create(
  ...
  virtual ~SandboxEnvAgentClient() = default;
  ...
  /// \param[in] serialized_sandbox_env The runtime
  /// environment serialized in JSON as from `SandboxEnv::Serialize` method.
  /// \param[in] callback The callback function.
  virtual void GetOrCreateSandboxEnv(const JobID &job_id,
                                     const std::string &serialized_sandbox_env,
                                     const rpc::SandboxEnvConfig &sandbox_env_config,
                                     GetOrCreateSandboxEnvCallback callback) = 0;

  /// Request agent to decrease the runtime env reference. This API is not idempotent. The
  /// client automatically retries on network errors.
  /// \param[in] serialized_sandbox_env The runtime environment serialized in JSON as from
  /// `SandboxEnv::Serialize` method.
  /// \param[in] callback The callback function.
  virtual void DeleteSandboxEnvIfPossible(
      const std::string &serialized_sandbox_env,
      DeleteSandboxEnvIfPossibleCallback callback) = 0;

  // NOTE: The service has another method `GetSandboxEnvsInfo` but nobody in raylet uses
  // it.
};
