...
// - connection has infinite timeout (This is because sandbox env agent can
...
inline constexpr std::string_view HTTP_PATH_GET_OR_CREATE_SANDBOX_ENV =
    "/get_or_create_sandbox_env";
inline constexpr std::string_view HTTP_PATH_DELETE_SANDBOX_ENV_IF_POSSIBLE =
...
class HttpSandboxEnvAgentClient : public SandboxEnvAgentClient {
 public:
  ...
  void ExitImmediately() {
    RAY_LOG(ERROR)
        << "The raylet exited immediately because the sandbox env agent timed out when "
           "Raylet try to connect to it. This can happen because the sandbox env agent "
    ...
    node_death_info.set_reason_message("Raylet could not connect to Sandbox Env Agent");
  ...
  /// successful but very long connection (e.g. sandbox env agent is busy downloading
  ...
  void RetryInvokeOnNotFoundWithDeadline(TryInvokeOnce<T> try_invoke_once,
    ...
    try_invoke_once(succ_callback, [=](ray::Status status) {
      if ((!status.IsNotFound()) && (!status.IsDisconnected())) {
        ...
        RAY_LOG(ERROR) << "Sandbox Env Agent timed out in " << agent_register_timeout_ms_
                       << "ms. Status: " << status << ", address: " << this->address_
                       << ", port: " << this->port_str_ << ", exiting immediately...";
        ExitImmediately();
      } else {
        RAY_LOG(INFO) << "Sandbox Env Agent network error: " << status
  ...
  void GetOrCreateSandboxEnv(const JobID &job_id,
                             const std::string &sandbox_runtime,
    ...
    RetryInvokeOnNotFoundWithDeadline<rpc::GetOrCreateSandboxEnvReply>(
        [=](SuccCallback<rpc::GetOrCreateSandboxEnvReply> succ_callback,
            FailCallback fail_callback) {
          return TryGetOrCreateSandboxEnv(
              job_id, sandbox_runtime, succ_callback, fail_callback);
        },
        /*succ_callback=*/
        [=](rpc::GetOrCreateSandboxEnvReply reply) {
          // HTTP request & protobuf parsing succeeded, but we got a non-OK from the
          // remote server.
          if (reply.status() != rpc::SANDBOX_AGENT_RPC_STATUS_OK) {
            RAY_LOG(INFO) << "Failed to create sandbox env for job " << job_id
                          << ", error message: " << reply.error_message();
            RAY_LOG(DEBUG) << "Serialized sandbox env for job " << job_id << ": "
                           << sandbox_runtime;
            std::vector<std::string> wrapper_command;
            callback(false,
                     reply.serialized_sandbox_env_context(),
                     /*setup_error_message*/ reply.error_message(),
                     wrapper_command);
          } else {
            RAY_LOG(INFO) << "Create sandbox env for job " << job_id;
            std::vector<std::string> wrapper_command(reply.wrapper_command().begin(),
                                                     reply.wrapper_command().end());
            callback(true,
                     reply.serialized_sandbox_env_context(),
                     /*setup_error_message*/ "",
                     wrapper_command);
        ...
        [=](ray::Status status) {
          std::string error_message = absl::StrCat(
              "Failed to create sandbox env for job ",
              job_id.Hex(),
              ", status = ",
              status.ToString(),
          ...
          RAY_LOG(DEBUG) << "Serialized sandbox env for job " << job_id << ": "
                         << sandbox_runtime;
          std::vector<std::string> wrapper_command;
          callback(false, "", error_message, wrapper_command);
        },
  ...
  void TryGetOrCreateSandboxEnv(
    ...
    request.set_sandbox_runtime(sandbox_runtime);
    ...
    auto session = Session::Create(
        ...
        port_str_,
        http::verb::post,
        HTTP_PATH_GET_OR_CREATE_SANDBOX_ENV,
        std::move(payload),
        /*succ_callback=*/
  ...
  void DeleteSandboxEnvIfPossible(const std::string &sandbox_runtime,
    ...
    RetryInvokeOnNotFoundWithDeadline<rpc::DeleteSandboxEnvIfPossibleReply>(
        [=](SuccCallback<rpc::DeleteSandboxEnvIfPossibleReply> succ_callback,
          ...
          return TryDeleteSandboxEnvIfPossible(
              sandbox_runtime, std::move(succ_callback), std::move(fail_callback));
        },
        /*succ_callback=*/
        [=](rpc::DeleteSandboxEnvIfPossibleReply reply) {
          if (reply.status() != rpc::SANDBOX_AGENT_RPC_STATUS_OK) {
            ...
            RAY_LOG(WARNING) << "Failed to delete sandbox env"
                             << ", error message: " << reply.error_message();
            RAY_LOG(DEBUG) << "Serialized sandbox env: " << sandbox_runtime;
            callback(false);
          } else {
            callback(true);
          }
        ...
        [=](ray::Status status) {
          RAY_LOG(WARNING)
              << "Failed to delete sandbox env reference, status = " << status
              << ", maybe there are some network problems, will fail the request.";
          RAY_LOG(DEBUG) << "Serialized sandbox env: " << sandbox_runtime;
          callback(false);
        },
  ...
  void TryDeleteSandboxEnvIfPossible(
    ...
    auto session = Session::Create(
        ...
        port_str_,
        http::verb::post,
        HTTP_PATH_DELETE_SANDBOX_ENV_IF_POSSIBLE,
        std::move(payload),
        /*succ_callback=*/
        [succ_callback, fail_callback](std::string body) {
          rpc::DeleteSandboxEnvIfPossibleReply reply;
